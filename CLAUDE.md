...
- SVDB modules use `label 'bcftools_svdb'` (combined bcftools+svdb container). Both per-sample collapse ([modules/local/svdb_collapse.nf](modules/local/svdb_collapse.nf)) and cohort merge ([modules/local/svdb_merge.nf](modules/local/svdb_merge.nf)) split each input VCF into DEL/DUP/INV (interval) and BND/INS subsets, run a separate `svdb --merge` on each subset with type-specific `--overlap`/`--bnd_distance` (`svdb_itvl_*` vs `svdb_bnd_*` params), then `bcftools concat --allow-overlaps` the two outputs. Unlike truvari (which splits one pre-merged multi-sample VCF), svdb splits each per-input file and merges the subset files itself; empty (header-only) subsets are valid svdb inputs and pass through normally. Collapse uses `--priority <callers>`; cohort merge does not.
