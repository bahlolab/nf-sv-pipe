...
Nextflow SV-calling pipeline. Entry: `main.nf` → workflow `CHORUS` in [workflows/sv_calling.nf](workflows/sv_calling.nf).

## Architecture

```
5 callers → MATCHA_COLLAPSE (per-sample) → MATCHA_MERGE (cohort)
```

Callers (MANTA, SMOOVE, CNVNATOR, DELLY, DELLY_CNV) each live in [subworkflows/local/](subworkflows/local/) and emit per-sample BCFs. Merge logic is in [subworkflows/local/merge.nf](subworkflows/local/merge.nf).
...
- `matcha` is a local binary at [bin/matcha](bin/matcha). Its modules use `container null` and invoke it directly — make sure `bin/` is on PATH (Nextflow handles this automatically for processes in this repo).
...
- **CNVNATOR** emits a text table; conversion to BCF goes via `cnvnator2VCF.pl` (in [bin/](bin/)) inside [modules/local/cnvnator_to_bcf.nf](modules/local/cnvnator_to_bcf.nf). Only `<DEL>` / `<DUP>` symbolic alleles.
